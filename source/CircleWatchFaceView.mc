...
class CircleWatchFaceView extends WatchUi.WatchFace {
    ...
    function onUpdate(dc as Dc) as Void {
        ...
        view.setText(Time.Gregorian.info(Time.now(), Time.FORMAT_MEDIUM).day_of_week + " "+Time.Gregorian.info(Time.now(), Time.FORMAT_MEDIUM).day);
    ...
    function drawWatchARC(dc, percentage, arcColor, limitColor, offset){
            ...
            dc.setPenWidth(arcPenWidth);
             if (percentage > 22) {
                ...
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_COUNTER_CLOCKWISE, 360-(3.2*(percentage-22)), 0);
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_CLOCKWISE, 70, 70-(3.2*percentage));

                arcRadius = screenRadius-1-offset - (arcPenWidth / 2);
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_COUNTER_CLOCKWISE, 359-(3.2*(percentage-22)), 1);
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_CLOCKWISE, 69, 70-(3.2*percentage));

                arcRadius = screenRadius-2-offset - (arcPenWidth / 2);
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_COUNTER_CLOCKWISE, 358-(3.2*(percentage-22)), 2);
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_CLOCKWISE, 68, 70-(3.2*percentage));
                ...
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_CLOCKWISE, 70, 70-(3.2*percentage)+1);

                arcRadius = screenRadius-1-offset - (arcPenWidth / 2);
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_CLOCKWISE, 69, 70-(3.2*percentage)+2);

                arcRadius = screenRadius-2-offset - (arcPenWidth / 2);
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_CLOCKWISE, 68, 70-(3.2*percentage));
                
                arcRadius = screenRadius-3-offset - (arcPenWidth / 2);
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_CLOCKWISE, 69, 70-(3.2*percentage)+1);
                
                arcRadius = screenRadius-4-offset - (arcPenWidth / 2);
                dc.drawArc(screenRadius, screenRadius, arcRadius, Graphics.ARC_CLOCKWISE, 70, 70-(3.2*percentage)+2);
             }
    }
    ...
    function drawStep(dc) {
        ...
        var activityInfo_stepGoal as Double.Number = activityInfo.stepGoal.toDouble();
        var steps_percentage as Double.Number = (activityInfo_steps/activityInfo_stepGoal);
    ...
    function drawSunriseSunset(){
        ...
        if (positionInfo != null && positionInfo.position != null) 
        {
            ...
            if(hasLocation)    
            {
                ...
                var timeString = Lang.format("$1$:$2$", [showTime.hour, showTime.min.format("%02d")]);
